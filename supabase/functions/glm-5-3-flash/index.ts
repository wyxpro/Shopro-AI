...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  // ── 1. 主通道：Sophnet 平台 GLM-5.3-Flash（OpenAI 兼容接口）────────────────
  // 密钥仅从环境变量读取：本地 supabase functions serve --env-file .env，生产走 Supabase Secrets，绝不硬编码
  ...
  if (glmApiKey) {
    ...
    try {
      const upstream = await fetch(`${glmBaseUrl}/v1/chat/completions`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          // 过小的预算会被思考耗尽导致 0 正文输出，此处预留 4096 思考+输出预算
      ...
      if (upstream.ok && upstream.body) {
        ...
        logProviderSwitch('provider_hit', { provider: 'sophnet', model: glmModel, latency_ms: latency, total_ms: Date.now() - reqStart });
        return streamResponse(upstream.body, 'sophnet', glmModel, latency);
      }

      logProviderSwitch('provider_fallback', { provider: 'sophnet', model: glmModel, status: upstream.status, latency_ms: Date.now() - nodeStart, next: 'dxkp' });
    } catch (glmErr) {
      logProviderSwitch('provider_error', { provider: 'sophnet', model: glmModel, error: String(glmErr), latency_ms: Date.now() - nodeStart, next: 'dxkp' });
    }
  } else {
    logProviderSwitch('provider_skipped', { provider: 'sophnet', reason: 'missing GLM_API_KEY' });
  }

  // ── 2. 备用通道：DeepSeek（dxkp 直连）────────────────────────────────────────
  const deepseekApiKey = Deno.env.get('DEEPSEEK_API_KEY') || Deno.env.get('API_KEY') || '';
  const deepseekBaseUrl = Deno.env.get('DEEPSEEK_BASE_URL') || 'https://ai.dxkp.com/v1';
  ...
  // ── 3. 高可用兜底：SiliconFlow 多模型链 ─────────────────────────────────────
  const siliconKey = Deno.env.get('SILICONFLOW_API_KEY') || '';
  if (!siliconKey) {
    logProviderSwitch('all_providers_unavailable', { reason: 'missing GLM_API_KEY / DEEPSEEK_API_KEY / SILICONFLOW_API_KEY', total_ms: Date.now() - reqStart });
    return new Response(
      JSON.stringify({ error: 'AI 服务未配置密钥，请在 Supabase Secrets 中配置 GLM_API_KEY / DEEPSEEK_API_KEY / SILICONFLOW_API_KEY。' }),
  ...
  return errorResponse('GLM service unavailable on all providers. Please try again later.', 502, undefined, req);
});
