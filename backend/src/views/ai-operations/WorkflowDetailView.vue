...
async function retry(job: JobDetail): Promise<void> {
  ...
  ElMessage.success('已创建新的执行尝试，正在等待执行结果')
  await load()
  // 不再依赖 window.setTimeout 魔法延时：显式轮询直到任务进入终态（重试由集中化副作用推进器模拟服务端回调）。
  await pollJobUntilSettled(job.id, (item) => item.status === 'SUCCEEDED' || item.status === 'FAILED' || item.status === 'TIMED_OUT' || item.status === 'EXPIRED', 8000)
}
...
async function requestCancel(job: JobDetail): Promise<void> {
  ...
  ElMessage.success('已提交取消请求，正在等待供应商确认')
  await load()
  await pollJobUntilSettled(job.id, (item) => item.status !== 'CANCEL_REQUESTED', 6000)
}

/** 有限轮询：每 700ms 重新拉取详情，直到目标任务满足收敛条件或超时，替代魔法延时。 */
async function pollJobUntilSettled(jobId: string, isSettled: (job: JobDetail) => boolean, maxMs: number): Promise<void> {
  const deadline = Date.now() + maxMs
  while (Date.now() < deadline) {
    await new Promise((resolve) => window.setTimeout(resolve, 700))
    await load()
    const job = detail.value?.jobs.find((item) => item.id === jobId)
...
async function submitManualRefund(): Promise<void> {
  ...
  try {
    ...
    ElMessage.error(err?.message || '人工补偿失败')
  } finally {
