...
import {approveRefund, createPlanVersion, getOrderSummary, getOrders, getPlans, getRefunds, rejectRefund} from '@/api/billing'
import EmptyState from '@/components/common/EmptyState.vue'
import PageHeader from '@/components/common/PageHeader.vue'
import StatusTag from '@/components/common/StatusTag.vue'
import {usePagedQuery} from '@/composables/usePagedQuery'
import {useAuthStore} from '@/stores/auth'
import type {Order, OrderListItem, OrderListQuery, OrderSummary, PlanSnapshot, RefundRequest} from '@/types'
...
// KPI 改由后端聚合端点提供（与列表同一筛选口径），不再前端拉 pageSize:200 自行汇总。
const metrics = computed(() => {
  const paidAmount = summary.value?.paidAmountMinor ?? 0
  const successfulRefundAmount = summary.value?.succeededRefundAmountMinor ?? 0
  const refundRate = summary.value?.refundRatePercent ?? 0
  const failedEntitlements = summary.value?.failedEntitlementCount ?? 0
  return [
    {label: '累计已支付金额', value: formatMoney(paidAmount, 'CNY'), hint: '后端聚合'},
    {label: '退款成功金额', value: formatMoney(successfulRefundAmount, 'CNY'), hint: '后端聚合'},
    {label: '退款率', value: `${refundRate.toFixed(1)}%`, hint: '退款成功金额 / 已支付金额'},
...
async function loadSummary(): Promise<void> {
  const {keyword, paymentStatus, entitlementStatus, userId} = orderQuery.value
  summary.value = (await getOrderSummary({keyword, paymentStatus, entitlementStatus, userId})).data
}

/** 筛选变化时同时刷新列表与后端聚合 KPI。 */
function loadScoped(): void {
  void loadOrders()
  void loadSummary()
}
...
async function approve(id: string): Promise<void> {
  ...
  try {
    ...
    await loadRefunds()
    await pollRefundSettlement()
  } catch (err: any) {
    ElMessage.error(err?.message || '退款审批操作失败')
  }
}

/** 退款执行改为服务端异步推进，前端以有限轮询读取终态（取代旧的魔法延时 setTimeout(load)）。 */
async function pollRefundSettlement(maxMs = 6000): Promise<void> {
  const start = Date.now()
  while (Date.now() - start < maxMs) {
    await new Promise((resolve) => setTimeout(resolve, 800))
    await Promise.all([loadRefunds(), loadSummary()])
    const stillSettling = refunds.value.some((refund) => refund.status === 'APPROVED' || refund.status === 'PROCESSING')
...
async function reject(id: string): Promise<void> {
  ...
  try {
    ...
    ElMessage.error(err?.message || '拒绝退款操作失败')
  }
}
...
async function submitPlan(): Promise<void> {
  ...
  try {
    ...
    ElMessage.error(err?.message || '创建套餐版本失败')
  } finally {
...
onMounted(async () => {
  ...
  await Promise.all([loadOrders(), loadSummary(), loadRefunds(), loadPlans()])
})
</script>
