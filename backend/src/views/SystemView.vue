      ...
      <div class="panel">
        <el-table :data="approvals" v-loading="approvalsLoading">
          ...
          <el-table-column label="状态" width="100">
            <template #default="{ row }">
              <StatusTag :label="approvalStatusLabel(row.status)" :type="approvalStatusType(row.status)"/>
            </template>
          </el-table-column>
          <el-table-column label="操作" width="150" fixed="right">
            <template #default="{ row }">
              <template v-if="row.status === 'PENDING' && canDecide(row)">
                <el-button link type="success" :disabled="!!decidingId" @click="decide(row, 'APPROVE')">通过</el-button>
                <el-button link type="danger" :disabled="!!decidingId" @click="decide(row, 'REJECT')">驳回</el-button>
              </template>
...
import {getAdmins, getApprovals, getAuditLogs, getRoles, getSystemHealth, resetDemoData, approveApproval, rejectApproval} from '@/api/system'
...
function approvalStatusLabel(status: ApprovalRequest['status']): string {
  return {PENDING: '待处理', APPROVED: '已通过', REJECTED: '已驳回'}[status]
}

function approvalStatusType(status: ApprovalRequest['status']): 'success' | 'warning' | 'danger' {
  return status === 'APPROVED' ? 'success' : status === 'REJECTED' ? 'danger' : 'warning'
}

/** 按审批类型判定当前用户是否可处置：REFUND 代理到退款审批权限，其余走审批处置权限。 */
function canDecide(row: ApprovalRequest): boolean {
  return row.type === 'REFUND' ? auth.hasPermission('billing:refund:approve') : auth.hasPermission('system:approvals:decide')
}

async function decide(row: ApprovalRequest, decision: 'APPROVE' | 'REJECT'): Promise<void> {
  const isApprove = decision === 'APPROVE'
  const {value} = await ElMessageBox.prompt(
      isApprove ? '通过后系统将按审批内容落账，可填写审批意见（可选）' : '请填写驳回原因',
      isApprove ? `通过审批 ${row.id}` : `驳回审批 ${row.id}`,
      {
        inputPlaceholder: isApprove ? '审批意见（可选）' : '驳回原因（必填）',
        inputValidator: (val: string) => (!isApprove && !val?.trim() ? '驳回必须填写原因' : true),
        type: isApprove ? 'success' : 'warning',
      },
  )
  decidingId.value = row.id
  try {
    const payload = {reason: value?.trim() || undefined}
    if (isApprove) await approveApproval(row.id, payload)
    else await rejectApproval(row.id, payload)
    ElMessage.success(isApprove ? '审批已通过' : '审批已驳回')
    await Promise.all([loadApprovals(), loadAudits()])
  } catch (error: unknown) {
    ElMessage.error(error instanceof Error ? error.message : '审批处置失败')
  } finally {
