...
import {createCreditAdjustment, CreditRuleError, reverseCreditEntry, updateUserCapabilities} from '../domain/credit-rules'
...
function isReversalRequest(value: unknown): value is CreateCreditReversalRequest {
...
export const customerHandlers = [
    ...
    http.patch('/api/admin/customers/users/:id/capabilities', async ({params, request}) => {
        const auth = authorize(request, PERMISSIONS.CUSTOMERS_OPERATE)
        if (auth instanceof Response) return auth
        ...
        try {
            const user = transactDemoDatabase((data) => {
                ...
                if (!existing) throw new CreditRuleError('用户不存在', 'NOT_FOUND')
                const before = {...existing.capabilities}
                const updated = updateUserCapabilities(data, userId, payload, actor)
                recordAudit(data, {
                    actor,
            ...
            return respondDomainError(error, '能力状态调整失败')
        }
    }),
    ...
    http.post('/api/admin/customers/users/:id/credit-adjustments', async ({params, request}) => {
        ...
        try {
            const result = transactDemoDatabase((data) => {
                ...
                const adjustment = createCreditAdjustment(data, userId, payload, actor)
            ...
            return respondDomainError(error, '积分调整失败')
        }
    }),
    http.post('/api/admin/customers/credit-ledger/:entryId/reversal', async ({params, request}) => {
        const auth = authorize(request, PERMISSIONS.CUSTOMERS_OPERATE)
        if (auth instanceof Response) return auth
        const actor = auth as AdminActor
        ...
        if (!entryId || !isReversalRequest(payload)) return validationError('积分冲正参数不正确')
        try {
            const entry = transactDemoDatabase((data) => {
                const original = data.creditLedger.find((item) => item.id === entryId)
                const reversal = reverseCreditEntry(data, entryId, payload, actor)
                recordAudit(data, {
                    actor,
                    action: `冲正积分流水 ${entryId}`,
                    targetType: 'CREDIT_LEDGER_ENTRY',
                    targetId: reversal.id,
                    reason: payload.reason,
                    before: original ? {balanceAfter: original.balanceAfter} : undefined,
                    after: {reversedEntryId: reversal.reversedEntryId, balanceAfter: reversal.balanceAfter},
                })
                return reversal
            })
            return ok(entry)
        } catch (error: unknown) {
            return respondDomainError(error, '积分冲正失败')
        }
    }),
    // 预留端点：组织列表/详情当前无独立管理页消费（客户详情已内联 organization 名称），保留供企业版多席位页面接入。
    ...
    http.get('/api/admin/customers/organizations/:id', ({params, request}) => {
        const auth = authorize(request, PERMISSIONS.CUSTOMERS_VIEW)
        if (auth instanceof Response) return auth
        ...
        if (!organization) return notFound('企业不存在')
