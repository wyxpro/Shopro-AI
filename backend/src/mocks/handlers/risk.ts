...
import type {AdminActor, CreateRiskDecisionRequest, RiskEvent, RiskEventListItem, RiskEventListQuery} from '@/types'
...
export const riskHandlers = [
    http.get('/api/admin/risk/events', ({request}) => {
        const auth = authorize(request, PERMISSIONS.RISK_VIEW)
        if (auth instanceof Response) return auth
        return ok(pageRiskEvents(readDemoDatabase(), queryFrom(new URL(request.url))))
    }),
    http.get('/api/admin/risk/events/:id', ({params, request}) => {
        const auth = authorize(request, PERMISSIONS.RISK_VIEW)
        if (auth instanceof Response) return auth
        ...
        if (!event) return notFound('风险事件不存在')
        const content = data.contents.find((item) => item.id === event.contentId)
        const user = data.users.find((item) => item.id === event.userId)
        if (!content || !user) return notFound('风险事件关联数据不存在')
    ...
    http.post('/api/admin/risk/events/:id/decisions', async ({params, request}) => {
        const auth = authorize(request, PERMISSIONS.RISK_DECIDE)
        if (auth instanceof Response) return auth
        ...
        try {
            const event = transactDemoDatabase((data) => {
                ...
                const updated = decideRiskEvent(data, eventId, payload, actor)
            ...
            return respondDomainError(error, '风险审核失败')
        }
    }),
    http.post('/api/admin/risk/events/:id/appeals', async ({params, request}) => {
        ...
        try {
            ...
            return respondDomainError(error, '登记申诉失败')
        }
    }),
]
