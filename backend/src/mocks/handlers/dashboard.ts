...
import {authorize, ok} from './utils'
...
export const dashboardHandlers = [
    http.get('/api/admin/dashboard', ({request}) => {
        const auth = authorize(request, PERMISSIONS.DASHBOARD_VIEW)
        if (auth instanceof Response) return auth
        return ok(dashboard(queryFrom(new URL(request.url))))
    }),
    // 预留端点：待办列表已内联于 /dashboard 的 todos 字段；此独立端点保留供外部/未来页面消费，暂不在管理页直接使用。
    http.get('/api/admin/dashboard/todos', ({request}) => {
        const auth = authorize(request, PERMISSIONS.DASHBOARD_VIEW)
        if (auth instanceof Response) return auth
        return ok(todosFor())
    }),
