...
import type {AdminActor, CreateSupportTicketRequest, SupportTicket, UpdateSupportTicketRequest} from '@/types'
...
export const ticketHandlers = [
    ...
    http.post('/api/admin/tickets', async ({request}) => {
        ...
        try {
            ...
            return respondDomainError(error, '创建工单失败')
        }
    }),
    http.get('/api/admin/tickets/:id', ({params, request}) => {
        const auth = authorize(request, PERMISSIONS.CUSTOMERS_VIEW)
        if (auth instanceof Response) return auth
    ...
    http.patch('/api/admin/tickets/:id', async ({params, request}) => {
        ...
        try {
            const ticket = transactDemoDatabase((data) => {
                ...
                if (!existing) throw new TicketRuleError('工单不存在', 'NOT_FOUND')
            ...
            return respondDomainError(error, '更新工单失败')
        }
    }),
]
