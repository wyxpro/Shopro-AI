import type {ApiCode, CreateSupportTicketRequest, SupportTicket, UpdateSupportTicketRequest} from '@/types'
import type {DemoDatabase} from '../db'
import {DomainError} from './errors'

export class TicketRuleError extends DomainError {
...
function requireTicket(data: DemoDatabase, ticketId: string): SupportTicket {
    ...
    if (!ticket) throw new TicketRuleError('工单不存在', 'NOT_FOUND')
    return ticket
}
...
export function createSupportTicket(data: DemoDatabase, payload: CreateSupportTicketRequest): SupportTicket {
    ...
    if (!data.users.some((item) => item.id === payload.userId)) throw new TicketRuleError('关联用户不存在', 'NOT_FOUND')
...
export function updateSupportTicket(
    ...
    if (payload.status === 'CLOSED' && !payload.resolution?.trim() && !ticket.resolution?.trim()) {
        throw new TicketRuleError('关闭工单前必须填写处理结论', 'BUSINESS_RULE_VIOLATION')
    }
