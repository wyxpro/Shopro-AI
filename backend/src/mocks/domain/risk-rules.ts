import type {AdminActor, ApiCode, ContentAsset, CreateRiskDecisionRequest, RiskEvent} from '@/types'
import type {DemoDatabase} from '../db'
import {DomainError} from './errors'

export class RiskRuleError extends DomainError {
    constructor(message: string, code: ApiCode = 'VALIDATION_ERROR') {
        super(code, message)
...
function requireContent(data: DemoDatabase, contentId: string): ContentAsset {
    ...
    if (!content) throw new RiskRuleError('关联内容不存在', 'NOT_FOUND')
    return content
}
...
function requireRiskEvent(data: DemoDatabase, eventId: string): RiskEvent {
    ...
    if (!event) throw new RiskRuleError('风险事件不存在', 'NOT_FOUND')
    return event
}
...
export function decideRiskEvent(
    ...
    if (!['PENDING_REVIEW', 'ESCALATED', 'APPEAL_PENDING'].includes(event.status)) {
        throw new RiskRuleError('当前风险事件不在可审核状态', 'CONFLICT')
    }
...
export function createRiskAppeal(data: DemoDatabase, eventId: string, message: string): RiskEvent {
    ...
    if (!['REJECTED', 'APPEAL_REJECTED'].includes(original.status)) {
        throw new RiskRuleError('仅已驳回的原风险事件可以登记申诉', 'CONFLICT')
    }
