import type {ApiCode, CreditLedgerEntry, Job, JobAttempt, LegacyJob, RetryJobResult, Workflow} from '@/types'
...
export class JobRuleError extends DomainError {
...
function requireJob(data: DemoDatabase, jobId: string): Job {
    ...
    if (!job) throw new JobRuleError('任务不存在', 'NOT_FOUND')
    return job
}
...
function requireWorkflow(data: DemoDatabase, workflowId: string): Workflow {
    ...
    if (!workflow) throw new JobRuleError('工作流不存在', 'NOT_FOUND')
    return workflow
}
...
export function retryJob(data: DemoDatabase, jobId: string): RetryJobResult {
    ...
    if (!previousAttempt || !previousAttempt.retryable) {
        throw new JobRuleError('该任务的错误类型不允许后台直接重试', 'BUSINESS_RULE_VIOLATION')
    }
...
export function refundFinalFailure(data: DemoDatabase, jobId: string, simulateFailure = false, actorId = 'system'): CreditLedgerEntry | undefined {
    const job = requireJob(data, jobId)
