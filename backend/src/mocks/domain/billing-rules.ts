...
export class BillingRuleError extends DomainError {
    constructor(message: string, code: ApiCode = 'CONFLICT') {
        super(code, message)
...
function requireOrder(data: DemoDatabase, orderId: string): Order {
    ...
    if (!order) throw new BillingRuleError('订单不存在', 'NOT_FOUND')
    return order
}
...
function requireRefund(data: DemoDatabase, refundId: string): RefundRequest {
    ...
    if (!refund) throw new BillingRuleError('退款申请不存在', 'NOT_FOUND')
    return refund
}

function requireUser(data: DemoDatabase, userId: string) {
    const user = data.users.find((item) => item.id === userId)
    if (!user) throw new BillingRuleError('订单所属用户不存在', 'NOT_FOUND')
    return user
}
...
function createOrderGrantEntry(data: DemoDatabase, order: Order, actor: AdminActor): CreditLedgerEntry | undefined {
...
export function regrantOrderEntitlement(data: DemoDatabase, orderId: string, actor: AdminActor): GrantEntitlementResult {
    ...
    const creditEntry = createOrderGrantEntry(data, order, actor)
...
export function createRefundRequest(data: DemoDatabase, payload: CreateRefundRequest, actor: AdminActor): RefundRequest {
    ...
    if (!Number.isInteger(payload.amountMinor) || payload.amountMinor <= 0 || payload.amountMinor > order.amountMinor) {
        throw new BillingRuleError('退款金额必须是订单金额范围内的最小货币单位整数', 'VALIDATION_ERROR')
    }
    if (!Number.isInteger(payload.reclaimCredits) || payload.reclaimCredits < 0 || payload.reclaimCredits > order.planSnapshot.grantedCredits) {
        throw new BillingRuleError('需回收积分不正确', 'VALIDATION_ERROR')
    }
    if (!payload.reasonCode.trim() || !payload.reason.trim()) throw new BillingRuleError('退款必须填写原因码和详细原因', 'VALIDATION_ERROR')
    const ticket = data.tickets.find((item) => item.id === payload.ticketId)
    if (!ticket || ticket.orderId !== order.id) throw new BillingRuleError('退款申请必须关联该订单的工单', 'VALIDATION_ERROR')
...
export function approveRefundRequest(data: DemoDatabase, refundId: string, reason: string, actor: AdminActor): RefundRequest {
    const refund = requireRefund(data, refundId)
    if (refund.status !== 'PENDING_APPROVAL') throw new BillingRuleError('仅待审批退款可以通过')
    if (!reason.trim()) throw new BillingRuleError('请填写退款审批意见', 'VALIDATION_ERROR')
    refund.status = 'APPROVED'
    refund.approverId = actor.id
    refund.updatedAt = new Date().toISOString()
    return refund
}

export function rejectRefundRequest(data: DemoDatabase, refundId: string, reason: string, actor: AdminActor): RefundRequest {
    const refund = requireRefund(data, refundId)
    if (refund.status !== 'PENDING_APPROVAL') throw new BillingRuleError('仅待审批退款可以拒绝')
    if (!reason.trim()) throw new BillingRuleError('请填写拒绝退款原因', 'VALIDATION_ERROR')
...
export function completeRefundExecution(data: DemoDatabase, refundId: string, actorId = 'system'): RefundRequest {
    const refund = requireRefund(data, refundId)
