...
/** 后端聚合订单 KPI（与列表同一筛选口径），取代前端拉 pageSize:200 自行汇总。 */
export function getOrderSummary(params: Partial<OrderListQuery>): Promise<ApiResponse<OrderSummary>> {
    return request<OrderSummary>({method: 'GET', url: '/admin/billing/summary', params})
}
